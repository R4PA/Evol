...
class Animal:
	def __init__(self,x,y,ID,color,partco=None,PK=False,health=50,maxspeed=RESO/10,eating=3,mutator=MUTATION,babyCount=1,matingtime=1):
		...
		self.healthColor=self.healthColorCalc()
		...
		#print "Creating: "+str(colors[self.color])
		self.shape=canvas.create_oval(x,y,x+RESO,y+RESO,fill=str(colors[self.color]))
		self.healthShape=canvas.create_oval(x+(0.2*RESO),y+(0.2*RESO),x+RESO-(0.2*RESO),y+RESO-(0.2*RESO),fill=str(self.healthColor))
		#self.lbl=str(self.ID)
		self.lbl=canvas.create_text(x+(RESO/2),y+(RESO/2),text="#"+str(self.ID),tags="text")
		self.draw()
		self.xspeed=0
...
#
	...
	def healthColorCalc(self):
		#print self.health
		pros=self.health*0.01
		#print "pros:"+str(pros)
		color=int(2*(pros*255))
		...
		if(color>255):
			colorGreenDes=255
			colorRedDes=(2*255)-color
		'''
		if(colorGreenDes<100):
			colorGreenDes='0'+str(colorGreenDes)
			if(colorGreenDes<100):
				colorGreenDes='0'+str(colorGreenDes)
		if(colorRedDes<100):
			colorRedDes='0'+str(colorRedDes)
			if(colorRedDes<100):
				colorRedDes='0'+str(colorRedDes)
		'''
		greenHex=hex(colorGreenDes)		
		redHex=hex(colorRedDes)
		greenHex=greenHex.lstrip('0x')
		redHex=redHex.lstrip('0x')
		for i in range(2):		
			if(len(greenHex)<2):
				greenHex="0"+str(greenHex)
			if(len(redHex)<2):
				redHex="0"+str(redHex)
		finalHealthColorInHex="#"+str(redHex)+str(greenHex)+"00"
		return finalHealthColorInHex
	...
	def draw(self):
		...
		canvas.delete(self.healthShape)
		self.healthShape=canvas.create_oval(pos[0]+int(0.2*RESO),pos[1]+int(0.2*RESO),pos[2]-int(0.2*RESO),pos[3]-int(0.2*RESO),fill=self.healthColor)
		self.drawLabel()
		return 0
#
	...
	def drawLabel(self):
		...
		self.lbl=canvas.create_text(pos[0]+(RESO/2),pos[1]+(RESO/2),font=("helvetica",LABELSIZE),fill="black",state=LABEL_STATE,text=str(self.ID),width=RESO*2,tags="text")
		canvas.delete(leib)	
		return 0
#
...
#
	...
	def move(self,vx,vy,rec=True):
		...
		canvas.move(self.healthShape,self.xspeed,self.yspeed)
